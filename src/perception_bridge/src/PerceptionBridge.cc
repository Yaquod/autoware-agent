...
PerceptionBridge::PerceptionBridge(rclcpp::Node::SharedPtr node, const std::shared_ptr<zenoh::Session>& zsession)
...
vehicle_frame::PerceptionFrame PerceptionBridge::buildFrame() {
  ...
  *frame.mutable_surrounding_objects() = {state_.surrounding_objects.begin(), state_.surrounding_objects.end()};
...
void PerceptionBridge::onSurroundingObject(autoware_perception_msgs::msg::PredictedObjects::SharedPtr msg) {
...
void PerceptionBridge::onTrafficSignal(autoware_perception_msgs::msg::TrafficLightGroupArray::SharedPtr msg) {
...
void PerceptionBridge::onSurroundingObjectImpl(const autoware_perception_msgs::msg::PredictedObjects::SharedPtr& msg) {
  ...
  for (const auto& object : msg->objects) {
    ...
    so.mutable_object_velocity()->set_vx_object(static_cast<float>(
    ...
    so.mutable_object_pose()->set_x(static_cast<float>(object.kinematics.initial_pose_with_covariance.pose.position.x));
    so.mutable_object_pose()->set_y(static_cast<float>(object.kinematics.initial_pose_with_covariance.pose.position.y));
    so.mutable_object_pose()->set_z(static_cast<float>(object.kinematics.initial_pose_with_covariance.pose.position.z));
    const auto& h = object.kinematics.initial_pose_with_covariance.pose.orientation;
    auto const TEMP = static_cast<float>(std::atan2(2.0F * (h.w * h.z + h.x * h.y), 1.0F - (2.0F * (h.y * h.y + h.z * h.z))));
    so.set_heading(TEMP);
...
void PerceptionBridge::onPointCloudImpl(const sensor_msgs::msg::PointCloud2::SharedPtr& msg) {
  pcl::PointCloud<pcl::PointXYZ>::Ptr const CLOUD = std::make_shared<pcl::PointCloud<pcl::PointXYZ>>();

  pcl::fromROSMsg(*msg, *CLOUD);

  pcl::PointCloud<pcl::PointXYZ>::Ptr  const DOWNSAMPLED_CLOUD = std::make_shared<pcl::PointCloud<pcl::PointXYZ>>();
...
void PerceptionBridge::onTrafficSignalImpl(const autoware_perception_msgs::msg::TrafficLightGroupArray::SharedPtr& msg) {
  ...
  for (const auto& group : msg->traffic_light_groups) {
    ...
    const auto& maxx =
      *std::ranges::max_element(group.elements,
