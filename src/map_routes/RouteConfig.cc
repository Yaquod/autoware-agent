...
namespace AutowareAgent {
    ...
    std::string RouteConfig::resolveConfigPath(const std::string& filename) {
        ...
        if (inputPath.is_absolute()) return inputPath.string();

        fs::path testPath = fs::path(AutowareAgent::TEST_MAP_DIR) / filename;
        if (fs::exists(testPath)) return testPath.string();

        fs::path srcPath = fs::path(AutowareAgent::SRC_MAP_DIR) / filename;
        if (fs::exists(srcPath)) return srcPath.string();

        fs::path installPath = fs::path(AutowareAgent::INSTALL_MAP_DIR) / filename;
        if (fs::exists(installPath)) return installPath.string();
