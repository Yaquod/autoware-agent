...
class RecordingGatewayServer final
...
public:
    ...
    bool wait_eta      (int n, std::chrono::milliseconds t = 5000ms) { return wait_(eta_count,       n, t); }
    bool wait_arrive   (int n, std::chrono::milliseconds t = 5000ms) { return wait_(arrive_count,    n, t); }
    bool wait_status   (int n, std::chrono::milliseconds t = 5000ms) { return wait_(status_count,    n, t); }
    bool wait_trip_init(int n, std::chrono::milliseconds t = 5000ms) { return wait_(trip_init_count, n, t); }
    bool wait_trip_move(int n, std::chrono::milliseconds t = 5000ms) { return wait_(trip_move_count, n, t); }
    bool wait_location (int n, std::chrono::milliseconds t = 5000ms) { return wait_(location_count,  n, t); }

    grpc::Status SendEta(grpc::ServerContext*,
                         const vehicle_gateway::EtaRequest* req,
                         vehicle_gateway::EtaResponse*) override
    {
        std::lock_guard lk(mtx); last_eta = *req; ++eta_count; cv.notify_all();
        return grpc::Status::OK;
    }

    grpc::Status SendArrive(grpc::ServerContext*,
                            const vehicle_gateway::ArriveRequest* req,
                            vehicle_gateway::ArriveResponse*) override
    {
        std::lock_guard lk(mtx); last_arrive = *req; ++arrive_count; cv.notify_all();
        return grpc::Status::OK;
    }

    grpc::Status SendStatus(grpc::ServerContext*,
                            const vehicle_gateway::StatusRequest* req,
                            vehicle_gateway::StatusResponse*) override
    {
        std::lock_guard lk(mtx); last_status = *req; ++status_count; cv.notify_all();
        return grpc::Status::OK;
    }

    grpc::Status TripInit(grpc::ServerContext*,
                          const vehicle_gateway::TripInitRequest* req,
                          vehicle_gateway::TripInitResponse*) override
    {
        std::lock_guard lk(mtx); last_trip_init = *req; ++trip_init_count; cv.notify_all();
        return grpc::Status::OK;
    }

    grpc::Status TripMove(grpc::ServerContext*,
                          const vehicle_gateway::TripMoveRequest* req,
                          vehicle_gateway::TripMoveResponse*) override
    {
        std::lock_guard lk(mtx); last_trip_move = *req; ++trip_move_count; cv.notify_all();
        return grpc::Status::OK;
    }

    grpc::Status UpdateVehicleLocation(
        grpc::ServerContext*,
        const vehicle_gateway::UpdateVehicleLocationRequest* req,
        vehicle_gateway::UpdateVehicleLocationResponse*) override
    {
        std::lock_guard lk(mtx); last_location = *req; ++location_count; cv.notify_all();
        return grpc::Status::OK;
    }
...
TEST_F(GatewayTest, ReportLocationTest) {
  ...
  // AutowareAgent::GPSCoordinate goal_gps = AutowareAgent::GPSCoordinate{35.68814679007944, 139.69440756809428};
  ...
  auto location_provider = std::make_shared<vehicle_gateway::ClusterLocationAdapter>(
                               frame, frame_mtx);
  auto eta_provider      = std::make_shared<vehicle_gateway::ClusterEtaAdapter>(
                               frame, frame_mtx, request_id);

  auto trip_manager_provider = std::make_shared<vehicle_gateway::AutowareControllerTripAdapter>(controller_);

  vehicle_gateway::VehicleGatewayService gateway(
      server_addr_,
      eta_provider,
      location_provider,
      trip_manager_provider,
      "MY-VIN",
      retry_io_);
...
TEST_F(GatewayTest, TripInitRpcRandomPoints) {
  ...
  size_t j = dist(gen);
  while (j == i && lanes.size() > 1) j = dist(gen);
